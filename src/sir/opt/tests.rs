...
// A load may find something different once something has written, so it is
// lifted out of a loop that writes nothing and left in one that does. The
// address is handed away first, which is what keeps the name in the frame and
// leaves a load here at all for the question to be asked of.
//
// What the loop does with the value is turn on it. A call would have answered
// the question before it was asked: calling something is one of the ways a
// loop may write.
#[test]
fn a_load_is_lifted_only_out_of_a_loop_that_writes_nothing() {
    let build = |writes: bool| {
        let mut f = Fixture::new();
        let x = f.local("x", f.bool);
        let c = f.param("c", f.bool);
        let (before, head, inner, more, exit) =
            (f.block(), f.block(), f.block(), f.block(), f.block());
        let yes = f.boolean(true);
        f.set(before, x, yes);
        let read = f.read(x);
        ...
        f.term(head, GIRTerm::Branch { cond, then: inner, els: exit });
        if writes {
            let no = f.boolean(false);
            f.set(inner, x, no);
        }
        let cond = f.read(x);
        f.term(inner, GIRTerm::Branch { cond, then: more, els: head });
        f.term(more, GIRTerm::Goto(head));
        f.term(exit, GIRTerm::Return(None));
        f.body(before);
        worked(f).0
    };
    ...
    assert!(
        ...
        "nothing writes, so the load is the same every turn: {:#?}",
