...
impl Parser {
    pub(super) fn build_items(
        ...
        Some(match rule_id {
            ...
            202 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            // <item_list> -> ε
            203 => self.here(ASTNodeKind::List(Vec::new())),
            // <item_list> -> <item_list> <item>
            204 => self.grew(c[0], c[1]),
            // <item_tail_opt> -> ε
            205 => self.here(ASTNodeKind::Empty),
            // <item_tail_opt> -> <attribute_list> <visibility_opt> <unterminated_decl>
            206 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            ...
            // ---- ASTVisibility ----------------------------------------------
            // <visibility> -> pub
            421 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Pub)), c[0]),
            // <visibility> -> priv
            422 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Priv)), c[0]),
            // <visibility> -> pub ( suite )
            423 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Suite)), c[0]),
            // <visibility_opt> -> ε
            424 => self.here(ASTNodeKind::Empty),
            ...
            // ---- where ---------------------------------------------------
            // <where_clause_opt> -> ε
            426 => self.here(ASTNodeKind::List(Vec::new())),
            // <where_clause_opt> -> where <where_pred_list>
            427 => self.pass(c[1]),
            // <where_pred> -> <where_subject> : <type_bounds>
            428 => self.at(ASTNodeKind::WherePred { ty: c[0], bounds: self.list(c[2]) }, c[0]),
            ...
            433 => self.at(ASTNodeKind::PtrType(c[1]), c[0]),
            // <where_subject> -> <base_type> <array_suffix_list>
            434 => self.fold_suffixes(c[0], c[1]),
            ...
            429 => self.at(ASTNodeKind::WherePred { ty: c[0], bounds: self.list(c[2]) }, c[0]),
            // <where_pred_list> -> <where_pred>
            430 => self.one(c[0]),
            // <where_pred_list> -> <where_pred_list> , <where_pred>
            431 => self.grew(c[0], c[2]),
            ...
            258 => self.at(ASTNodeKind::NamedPayload(self.list(c[1])), c[0]),
            ...
            364 => self.grew(c[0], c[1]),
            // <trait_tail_opt> -> ε
            365 => self.here(ASTNodeKind::Empty),
            // <trait_tail_opt> -> <attribute_list> <fn_sig>
            366 => self.with_attrs(c[1], c[0], ASTVisibility::Unwritten),
            ...
            182 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            // <impl_member_list> -> ε
            183 => self.here(ASTNodeKind::List(Vec::new())),
            // <impl_member_list> -> <impl_member_list> <impl_member>
            184 => self.grew(c[0], c[1]),
            // <impl_tail_opt> -> ε
            185 => self.here(ASTNodeKind::Empty),
            // <impl_tail_opt> -> <attribute_list> <visibility_opt> <fn_sig>
            186 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            ...
            189 | 190 => self.pass(c[0]),
            // <import_path> -> <path_seg>
            191 => self.at(ASTNodeKind::Name(vec![self.text(c[0])]), c[0]),
            ...
            // <var_intro> -> let
            413 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Let)), c[0]),
            // <var_intro> -> var
            414 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Var)), c[0]),
