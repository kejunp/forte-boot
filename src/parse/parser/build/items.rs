...
impl Parser {
    pub(super) fn build_items(
        ...
        Some(match rule_id {
            ...
            // ---- ASTVisibility ----------------------------------------------
            // <visibility> -> pub
            425 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Pub)), c[0]),
            // <visibility> -> priv
            426 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Priv)), c[0]),
            // <visibility> -> pub ( suite )
            427 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Suite)), c[0]),
            // <visibility_opt> -> ε
            428 => self.here(ASTNodeKind::Empty),
            ...
            145 => self.at(ASTNodeKind::Empty, c[0]),
            ...
            149 => self.with_modifier(c[1], c[0], true, false),
            // <fn_sig> -> unsafe <fn_head>
            150 => self.with_modifier(c[1], c[0], false, true),
            // <fn_sig> -> const unsafe <fn_head>
            151 => self.with_modifier(c[2], c[0], true, true),
            ...
            234 => self.grew(c[0], c[2]),
            // <macro_param_list_opt> -> ε
            235 => self.here(ASTNodeKind::List(Vec::new())),
            ...
            338 => self.at(ASTNodeKind::SelfRecv(ASTSelf::Value, None), c[0]),
            ...
            164 => self.grew(c[0], c[2]),
            // <generic_args_opt> -> ε
            166 => self.here(ASTNodeKind::List(Vec::new())),
            ...
            // ---- where ---------------------------------------------------
            // <where_clause_opt> -> ε
            430 => self.here(ASTNodeKind::List(Vec::new())),
            // <where_clause_opt> -> where <where_pred_list>
            431 => self.pass(c[1]),
            // <where_pred> -> <where_subject> : <type_bounds>
            432 => self.at(ASTNodeKind::WherePred { ty: c[0], bounds: self.list(c[2]) }, c[0]),
            ...
            437 => self.at(ASTNodeKind::PtrType(c[1]), c[0]),
            // <where_subject> -> <base_type> <array_suffix_list>
            438 => self.fold_suffixes(c[0], c[1]),
            ...
            433 => self.at(ASTNodeKind::WherePred { ty: c[0], bounds: self.list(c[2]) }, c[0]),
            // <where_pred_list> -> <where_pred>
            434 => self.one(c[0]),
            // <where_pred_list> -> <where_pred_list> , <where_pred>
            435 => self.grew(c[0], c[2]),
            ...
            127 => self.grew(c[0], c[2]),
            // <field_decl_list_opt> -> ε
            128 => self.here(ASTNodeKind::List(Vec::new())),
            ...
            // <elif_list> -> ε
            103 => self.here(ASTNodeKind::List(Vec::new())),
            ...
            // <else_opt> -> ε
            105 => self.here(ASTNodeKind::Empty),
            // <else_opt> -> else <block>
            106 => self.pass(c[1]),
            ...
            110 => self.grew(c[0], c[2]),
            // <enum_variant_list_opt> -> ε
            111 => self.here(ASTNodeKind::List(Vec::new())),
            ...
            260 => self.at(ASTNodeKind::NamedPayload(self.list(c[1])), c[0]),
            ...
            // is theirs.
            364 => self.with_attrs(c[1], c[0], ASTVisibility::Unwritten),
            // <trait_member_list> -> ε
            365 => self.here(ASTNodeKind::List(Vec::new())),
            // <trait_member_list> -> <trait_member_list> <trait_member>
            366 => self.grew(c[0], c[1]),
            // <trait_tail_opt> -> ε
            367 => self.here(ASTNodeKind::Empty),
            // <trait_tail_opt> -> <attribute_list> <fn_sig>
            368 => self.with_attrs(c[1], c[0], ASTVisibility::Unwritten),
            ...
            // <impl_for_opt> -> ε
            182 => self.here(ASTNodeKind::Empty),
            // <impl_for_opt> -> for <type>
            183 => self.pass(c[1]),
            // <impl_member> -> <attribute_list> <visibility_opt> <fn_decl>
            184 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            // <impl_member_list> -> ε
            185 => self.here(ASTNodeKind::List(Vec::new())),
            // <impl_member_list> -> <impl_member_list> <impl_member>
            186 => self.grew(c[0], c[1]),
            // <impl_tail_opt> -> ε
            187 => self.here(ASTNodeKind::Empty),
            // <impl_tail_opt> -> <attribute_list> <visibility_opt> <fn_sig>
            188 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            ...
            191 | 192 => self.pass(c[0]),
            // <import_path> -> <path_seg>
            193 => self.at(ASTNodeKind::Name(vec![self.text(c[0])]), c[0]),
            ...
            // <var_head> -> <var_intro> <tuple_pattern> <type_annotation_opt> <initializer_opt>
            //
            // No `<gc_opt>`: a `gc` binding holds one handle, and there is
            // nothing for one pattern to take several of it. The type
            // annotation is the whole tuple's, the way it is on the name form.
            416 => {
                let intro = intro_of(self.mark(c[0]));
                self.at(
                    ASTNodeKind::Variable {
            ...
            // <var_intro> -> let
            417 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Let)), c[0]),
            // <var_intro> -> var
            418 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Var)), c[0]),

            // ---- gc ------------------------------------------------------
            // Read by `<var_head>` above through `opt`, which is why the word
            // reduces to a mark and its absence to an `Empty`.
            // <gc_opt> -> ε
            158 => self.here(ASTNodeKind::Empty),
            // <gc_opt> -> gc
            159 => self.at(ASTNodeKind::Mark(ASTMark::Gc), c[0]),

            // ---- The optional semicolon ----------------------------------
            // Nothing above reads it: it is written for the grammar, which has
            // to say that it may be there.
            // <semi_opt> -> ε
            345 => self.here(ASTNodeKind::Empty),
            // <semi_opt> -> ;
            346 => self.at(ASTNodeKind::Empty, c[0]),

            // ---- What a `;` may be left off ------------------------------
            // <unterminated_decl> -> <var_head> | <const_head> | <type_head>
            //                     |  <import_head> | <fn_sig>
            401 | 402 | 403 | 404 | 405 => self.pass(c[0]),
            // <unterminated_stmt> -> <expression> | <var_head> | <const_head>
            //                     |  <type_head>
            406 | 407 | 408 | 409 => self.pass(c[0]),
            // <unterminated_stmt> -> unsafe <unterminated_stmt>
            410 => self.at(ASTNodeKind::Unsafe(c[1]), c[0]),
