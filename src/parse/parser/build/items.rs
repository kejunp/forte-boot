...
impl Parser {
    pub(super) fn build_items(
        ...
        Some(match rule_id {
            ...
            203 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            // <item_list> -> ε
            204 => self.here(ASTNodeKind::List(Vec::new())),
            // <item_list> -> <item_list> <item>
            205 => self.grew(c[0], c[1]),
            // <item_tail_opt> -> ε
            206 => self.here(ASTNodeKind::Empty),
            // <item_tail_opt> -> <attribute_list> <visibility_opt> <unterminated_decl>
            207 => self.with_attrs(c[2], c[0], self.visibility(c[1])),
            ...
            // ---- ASTVisibility ----------------------------------------------
            // <visibility> -> pub
            424 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Pub)), c[0]),
            // <visibility> -> priv
            425 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Priv)), c[0]),
            // <visibility> -> pub ( suite )
            426 => self.at(ASTNodeKind::Mark(ASTMark::Vis(ASTVisibility::Suite)), c[0]),
            // <visibility_opt> -> ε
            427 => self.here(ASTNodeKind::Empty),
            ...
            172 => self.grew(c[0], c[2]),
            // <generic_params> -> < <generic_param_list> >
            173 => self.pass(c[1]),
            // <generic_params_opt> -> ε
            174 => self.here(ASTNodeKind::List(Vec::new())),
            // <generic_params_opt> -> <generic_params>
            175 => self.pass(c[0]),

            // ---- where ---------------------------------------------------
            // <where_clause_opt> -> ε
            429 => self.here(ASTNodeKind::List(Vec::new())),
            // <where_clause_opt> -> where <where_pred_list>
            430 => self.pass(c[1]),
            // <where_pred> -> <where_subject> : <type_bounds>
            431 => self.at(ASTNodeKind::WherePred { ty: c[0], bounds: self.list(c[2]) }, c[0]),
            ...
            436 => self.at(ASTNodeKind::PtrType(c[1]), c[0]),
            // <where_subject> -> <base_type> <array_suffix_list>
            437 => self.fold_suffixes(c[0], c[1]),
            ...
            259 => self.at(ASTNodeKind::NamedPayload(self.list(c[1])), c[0]),
            ...
            // is theirs.
            363 => self.with_attrs(c[1], c[0], ASTVisibility::Unwritten),
            // <trait_member_list> -> ε
            364 => self.here(ASTNodeKind::List(Vec::new())),
            // <trait_member_list> -> <trait_member_list> <trait_member>
            365 => self.grew(c[0], c[1]),
            // <trait_tail_opt> -> ε
            366 => self.here(ASTNodeKind::Empty),
            // <trait_tail_opt> -> <attribute_list> <fn_sig>
            367 => self.with_attrs(c[1], c[0], ASTVisibility::Unwritten),
            ...
            // <impl_member_list> -> ε
            184 => self.here(ASTNodeKind::List(Vec::new())),
            // <impl_member_list> -> <impl_member_list> <impl_member>
            185 => self.grew(c[0], c[1]),
            // <impl_tail_opt> -> ε
            186 => self.here(ASTNodeKind::Empty),
            ...
            190 | 191 => self.pass(c[0]),
            // <import_path> -> <path_seg>
            192 => self.at(ASTNodeKind::Name(vec![self.text(c[0])]), c[0]),
            ...
            // <var_intro> -> let
            416 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Let)), c[0]),
            // <var_intro> -> var
            417 => self.at(ASTNodeKind::Mark(ASTMark::Intro(ASTVariableIntro::Var)), c[0]),

            // ---- gc ------------------------------------------------------
            // Read by `<var_head>` above through `opt`, which is why the word
            // reduces to a mark and its absence to an `Empty`.
            // <gc_opt> -> ε
            157 => self.here(ASTNodeKind::Empty),
            // <gc_opt> -> gc
            158 => self.at(ASTNodeKind::Mark(ASTMark::Gc), c[0]),

            // ---- The optional semicolon ----------------------------------
            // Nothing above reads it: it is written for the grammar, which has
            // to say that it may be there.
            // <semi_opt> -> ε
            344 => self.here(ASTNodeKind::Empty),
            // <semi_opt> -> ;
            345 => self.at(ASTNodeKind::Empty, c[0]),

            // ---- What a `;` may be left off ------------------------------
            // <unterminated_decl> -> <var_head> | <const_head> | <type_head>
            //                     |  <import_head> | <fn_sig>
            400 | 401 | 402 | 403 | 404 => self.pass(c[0]),
            // <unterminated_stmt> -> <expression> | <var_head> | <const_head>
            //                     |  <type_head>
            405 | 406 | 407 | 408 => self.pass(c[0]),
            // <unterminated_stmt> -> unsafe <unterminated_stmt>
            409 => self.at(ASTNodeKind::Unsafe(c[1]), c[0]),
