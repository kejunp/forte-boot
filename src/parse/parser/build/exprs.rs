...
impl Parser {
    pub(super) fn build_exprs(
        ...
        Some(match rule_id {
            ...
            // <range_op> -> ..
            333 => self.at(ASTNodeKind::Mark(ASTMark::Range(ASTRangeOp::Exclusive)), c[0]),
            // <range_op> -> ..=
            334 => self.at(ASTNodeKind::Mark(ASTMark::Range(ASTRangeOp::Inclusive)), c[0]),
            ...
            // <equality_op> -> ==
            116 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Eq)), c[0]),
            // <equality_op> -> !=
            117 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Ne)), c[0]),
            ...
            // <shift_op> -> <<
            352 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Shl)), c[0]),
            // <shift_op> -> >>
            353 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Shr)), c[0]),
            ...
            // <multiplicative_op> -> *
            255 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Mul)), c[0]),
            // <multiplicative_op> -> /
            256 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Div)), c[0]),
            // <multiplicative_op> -> %
            257 => self.at(ASTNodeKind::Mark(ASTMark::Bin(ASTBinOp::Rem)), c[0]),
            ...
            390 => self.pass(c[0]),
            // <unary_op> -> !
            391 => self.at(ASTNodeKind::Mark(ASTMark::Unary(ASTUnaryOp::Not)), c[0]),
            // <unary_op> -> -
            392 => self.at(ASTNodeKind::Mark(ASTMark::Unary(ASTUnaryOp::Neg)), c[0]),
            // <unary_op> -> <ref_op>
            ...
            // <unary_op> -> addr
            394 => self.at(ASTNodeKind::Mark(ASTMark::Unary(ASTUnaryOp::Addr)), c[0]),
            // <unary_op> -> deref
            395 => self.at(ASTNodeKind::Mark(ASTMark::Unary(ASTUnaryOp::Deref)), c[0]),
            ...
            // <postfix_op> -> [ <index> ]
            293 => self.at(ASTNodeKind::Index { base: HOLE, index: c[1] }, c[0]),
            ...
            //           |  <tuple_expr>
            296 | 297 | 300 | 303 | 304 | 305 | 306 | 307 => self.pass(c[0]),
            ...
            // <primary> -> super
            298 => self.at(ASTNodeKind::Ident("super".to_string()), c[0]),
            // <primary> -> suite
            299 => self.at(ASTNodeKind::Ident("suite".to_string()), c[0]),
            ...
            // ---- Closures, continued -------------------------------------
            // <move_opt> -> ε
            251 => self.here(ASTNodeKind::Empty),
            // <move_opt> -> move
            252 => self.at(ASTNodeKind::Mark(ASTMark::Move), c[0]),

            // ---- Jumps ---------------------------------------------------
            // <jump_expr> -> return <expression_opt>
            207 => self.at(ASTNodeKind::Return(self.opt(c[1])), c[0]),
            // <jump_expr> -> break <expression_opt>
            208 => self.at(ASTNodeKind::Break(self.opt(c[1])), c[0]),
            // <jump_expr> -> continue
            209 => self.at(ASTNodeKind::Continue, c[0]),
            ...
            213 | 214 | 215 | 216 | 217 | 218 | 219 => self.pass(c[0]),
            // <literal_pattern> -> <literal>
            220 => self.at(ASTNodeKind::LitPat { negated: false, value: self.lit(c[0]) }, c[0]),
            // <literal_pattern> -> - <literal>
            221 => self.at(ASTNodeKind::LitPat { negated: true, value: self.lit(c[1]) }, c[0]),

            // ---- Values --------------------------------------------------
            // <value_expr> -> <assignment> | <closure_expr> | <block_expr>
            408 | 409 | 410 => self.pass(c[0]),
            ...
            269 => self.pass(c[0]),
            // <path_seg> -> suite
            270 => self.at(ASTNodeKind::Ident("suite".to_string()), c[0]),
            // <path_seg> -> super
            271 => self.at(ASTNodeKind::Ident("super".to_string()), c[0]),
            // <path_seg> -> self
            272 => self.at(ASTNodeKind::Ident("self".to_string()), c[0]),

            // <qualified_name> -> <path_seg>
            326 => self.at(ASTNodeKind::Name(vec![self.text(c[0])]), c[0]),
            ...
            118 => self.at(ASTNodeKind::ExprStmt(c[0]), c[0]),
            // <expression> -> <value_expr> | <jump_expr>
            119 | 120 => self.pass(c[0]),
            // <expression_opt> -> ε
            121 => self.here(ASTNodeKind::Empty),
            // <expression_opt> -> <expression>
            122 => self.pass(c[0]),
            // <expression_seq> -> <expression>
            123 => self.one(c[0]),
            // <expression_seq> -> <expression_seq> , <expression>
            124 => self.grew(c[0], c[2]),
